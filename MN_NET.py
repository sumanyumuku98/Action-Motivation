...
class MN_NET(torch.nn.module):
	...
	def __init__(self, batch_size, seq_len=1,num_layers=1, action_categories=0):
    ...
    def forward(img,h_init= self.h_init):
    	img = self.resnet(img)
       	...
       	input_seq = torch.autograd.Variable(torch.zeros(self.seq_len,batch_size,256), requires_grad = True)
       	for i in range(input_seq.shape[0]):
       		input_seq[i,:,:] = img[:,:]

        output, h_out = self.gru(input_seq,h_init)
